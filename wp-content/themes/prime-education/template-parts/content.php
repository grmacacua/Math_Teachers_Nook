...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
	 ...
	 if ( $prime_education_image_setting ){ 
		 echo ( !is_single() ) ? '<a href="' . esc_url( get_the_permalink() ) . '" class="post-thumbnail">' : '<div class="post-thumbnail">'; ?>
	 			<?php ( is_active_sidebar( 'right-sidebar' ) ) ? the_post_thumbnail( 'prime-education-with-sidebar', array( 'itemprop' => 'image' ) ) : the_post_thumbnail( 'prime-education-without-sidebar', array( 'itemprop' => 'image' ) ) ; ?>
    ...
    <?php } ?>
    <?php if ( !is_single() && $prime_education_read_more_setting ) { ?>
