...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
	 ...
	 <?php
			// Get the post ID
			$prime_education_post_id = get_the_ID();

			// Check if there are videos embedded in the post content
			$prime_education_post = get_post($prime_education_post_id);
			$prime_education_content = do_shortcode(apply_filters('the_content', $prime_education_post->post_content));
			$prime_education_embeds = get_media_embedded_in_content($prime_education_content);

			if (!empty($prime_education_embeds)) {
			    // Loop through embedded media and display videos
			    foreach ($prime_education_embeds as $embed) {
